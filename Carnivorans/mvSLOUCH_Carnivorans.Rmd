...
computational engine provided by PCMBase [@VMitKBarGAsiTSta2020], so let us start by attaching mvSLOUCH as well as geiger 
[@LHaretalGEIGER] and ggplot2 [@HWic2016], which have some useful functions for data processing and plotting, respectively
...
We preload them so that building the vignette it does not take over a day and we store with mvSLOUCH only the necessary
...
We will explore these ideas using a subset of the dataset collected in [@JSamJMeaSSak2013],
available at the Dryad Data Repository dx.doi.org/10.5061/dryad.77tm4. We first download the data, remove fossil samples 
(lacking locomotor ecology data, one, *Urocyon cinereoargenteus*, has to be removed separately as the contemporary and 
fossil entry have the same species name), species with missing measurements (for radius length, see below) and 
*Lycalopex sp.* (as species identification is required for branch length assignation), 
rename species according to @WJohetal2006 and @DWilDRee2005, and matching the tip 
...
$105$ species after removing those which phenotypic data were not complete, defined at the species level, 
...
You will notice that the number of internal nodes $(90)$ is unexpectedly low given the number of tips $(105)$. 
...
in fact can result in more stabilize estimations than when using phylogenies with very short branches (close to zero). 
...
plot(ScaledTree, cex = 0.3,  edge.color = reg.col, edge.width=3.5)
```

```{r eval=TRUE, echo=FALSE, out.width = "100%", fig.pos="h"}
knitr::include_graphics("./ScaledTree.png", auto_pdf=TRUE)
```
...
Key parameter estimates for this model are the trait covariance matrix and the ancestral trait values:
...
evolution of their limbs, several regimes should be considered (one for each habitat type, analogous to ANCOVA). 
...
radius length.
So, the response variables (especially radius length) are less influenced by evolving humerus length
when locomotor types are accounted for. But what if humerus length does not evolve independently of the other two variables? 
...
(essentially instantaneous adaptation) and one extremely long half-lie

```{r}
OUBMestim$FinalFound$ParamSummary$phyl.halflife$halflives
```

Such situations cause numerical issues, when summarizing parameters calculating summary statistics or conditional
distributions. 
...
The reason why we calculate the conditional $\mathrm{R}^{2}$ without the phylogeny is because the linear
...
Let us try a model with the matrices  
$\mathbf{\Sigma}_{yy}$ as lower triangular, and $\mathbf{A}$ as diagonal: 
...
The model setting of this preferred candidate behaves much better than the OUOU model under a global regime we fitted earlier 
...
different parameterization results in a much simpler structure, $\mathbf{A}$ is diagonal
...
(AICc = $-272.2136$), although the difference of values in these two comparisons is considerable: 
...
($\Delta$AICc $= 7.726385$), and essentially no support for the OUOU accounting for locomotor types under default settings 
($\Delta$AICc $= 241.6319$), when compared with the preferred candidate of the new comparison (`OUf$BestModel`). 
...
```
```{r eval=FALSE, echo=TRUE}
plot(ScaledTree, cex = 0.3, edge.color = climb.col, edge.width=3.5)
```

```{r eval=TRUE, echo=FALSE, out.width = "100%", fig.pos="h"}
knitr::include_graphics("./ScaledTree2.png", auto_pdf=TRUE)
```
...
does better than the global regime (AICc = $-272.2136$), but not as well as the full regime specification (AICc = $-283.5499$):
...
```
```{r eval=FALSE, echo=TRUE}
plot(ScaledTree, cex = 0.3, edge.color = strok.col, edge.width=3.5)
```

```{r eval=TRUE, echo=FALSE, out.width = "100%", fig.pos="h"}
knitr::include_graphics("./ScaledTree3.png", auto_pdf=TRUE)
```
...
This model does better than the alternative lumped strategy (AICc = $-278.0746$), but only extremely marginally better than the 
full regime specification (AICc = $-283.5499$): 
...
```
```{r eval=FALSE, echo=TRUE}
plot(ScaledTree, cex = 0.3, edge.color = red.col, edge.width=3.5)
```

```{r eval=TRUE, echo=FALSE, out.width = "100%", fig.pos="h"}
knitr::include_graphics("./ScaledTree4.png", auto_pdf=TRUE)
```
...
But it is not as well supported as the previous model (AICc = $-283.5876$) or as the full regime specification (AICc = $-283.5499$):
...
selective regime than the particular medium being pulled (water or soil). $\mathrm{R}^{2}$ is high, indicating that the model 
explains considerable variation in the data:
