...
OUOUbootstrap_model4_sDiagonal_var <- parametric.bootstrap(estimated.model=OUOUmodel, phyltree=ferulatreeape, values.to.bootstrap=c("mPsi", "trait.regression", "corr.matrix", "conditional.corr.matrix", "phyl.halflife"), regimes=NULL, M.error=M.error, predictors=NULL , kY=NULL, numboot=1000, Atype="Any", Syytype="Diagonal", diagA=NULL, parameter_signs=list(signsA=rbind(c("+",NA,"0","0","0"),c(NA,"+","0","0","0"),c("0","0","+",NA,"0"),c("0","0",NA,"+","0"),c("0","0","0","0","+"))))

# write the full output ot the file

capture.output(OUOUbootstrap_model4_sDiagonal_var,file = "full_output_boot_model4_sDiagonal_var.txt")

# CI for trait regression

y <- matrix(,nrow=length(OUOUbootstrap_model4_sDiagonal_var$bootstrapped.parameters$trait.regression), ncol=length(OUOUmodel$FinalFound$ParamSummary$trait.regression[[1]]))
...
for(i in 1:length(OUOUmodel$FinalFound$ParamSummary$trait.regression)){
       x <- sapply(OUOUbootstrap_model4_sDiagonal_var$bootstrapped.parameters$trait.regression,function(x) x[i])
...
print(Output_tr)
capture.output(Output_tr, file = "CI_trait.regression_model4_sDiagonal_var.txt")
...
for(i in 1:length(as.vector(OUOUmodel$FinalFound$ParamSummary$corr.matrix))){
  BT.CorrMat<-quantile(sapply(OUOUbootstrap_model4_sDiagonal_var$bootstrapped.parameters$corr.matrix,function(x) x[i]),c(0.025,0.975))
...
print(Output_cm)
capture.output(Output_cm, file = "CI_correlation.matrix_model4_sDiagonal_var.txt")
...
BT_halflives <- vector(mode="list", length=length(OUOUbootstrap_model4_sDiagonal_var$bootstrapped.parameters$phyl.halflife))

for(i in 1:length(OUOUbootstrap_model4_sDiagonal_var$bootstrapped.parameters$phyl.halflife)) {
          BT_halflives[[i]] <- OUOUbootstrap_model4_sDiagonal_var$bootstrapped.parameters$phyl.halflife[[i]]$halflives
          }
...
print(Output_hl)
capture.output(Output_hl, file = "CI_halflives_model4_sDiagonal_var.txt")
