	...
	sapply(lsetup$simulsetup$N,function(N,lsetup,runnum,main_directory,setuprun_directory){
		...
		lres<-sapply(v_iter,function(i,N,lsetup,setup_comments_file,setup_error_comments_file,setuprun_directory,setuprun_directory_tmp,b_use_random_seed_from_manuscript,b_should_random_seed_be_saved){
			...
			res_i<-NA
				    ...
				    bm=mvSLOUCH::simulBMProcPhylTree(ape_phyltree,X0=lsetup$simulsetup$modelparams$vX0,Sigma=lsetup$simulsetup$modelparams$Sxx),
    				    ouch=mvSLOUCH::simulOUCHProcPhylTree(ape_phyltree,lsetup$simulsetup$modelparams),
    				    mvslouch=mvSLOUCH::simulMVSLOUCHProcPhylTree(ape_phyltree,lsetup$simulsetup$modelparams)
    			    )
			    ...
			    s_time<-proc.time()
	    		    ...
	    		    mvsl_estim_res<-mvSLOUCH::estimate.evolutionary.model(ape_phyltree, mData, regimes = NULL, 
    	    			    root.regime = NULL, M.error = NULL, repeats =  lsetup$recoverunder$repeats, model.setups = lsetup$recoverunder$model_setups, 
			    ...
			    lextracted_estim_summary<-extract_estimation_summary(lsetup$simulsetup,lsetup$recoverunder$model_setups,mvsl_estim_res)
	    		    true_param_summary<-switch(lsetup$simulsetup$evolmodel,
		    		    bm=mvSLOUCH::SummarizeBM(phyltree=ape_phyltree, mData=mData, modelParams=lextracted_estim_summary$TruePoint_model, t = c(1)),
				    ouch=mvSLOUCH::SummarizeOUCH(phyltree=ape_phyltree, mData=mData, modelParams=lextracted_estim_summary$TruePoint_model, t = c(1)),	
				    mvslouch=mvSLOUCH::SummarizeMVSLOUCH(phyltree=ape_phyltree, mData=mData, modelParams=lextracted_estim_summary$TruePoint_model, t = c(1))
			    )
		    ...
		    res_i<-list(ape_phyltree=ape_phyltree,mData=mData,mvsl_estim_res=mvsl_estim_res,lextracted_estim_summary=lextracted_estim_summary,true_param_summary=true_param_summary,runtime=runtime,i=i)
