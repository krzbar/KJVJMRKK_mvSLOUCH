...
f_findvalueinlist<-function(lmodelparams,vlocation){
    ...
    for (cfieldname in vlocation){
        ...
        ltruelist<-ltruelist[[which(names(ltruelist)==cfieldname)]]
    }
    ltruelist    
}
...
fL2_relative<-function(x,y){x<-c(x);y<-c(y);fdistL2scalarproduct_relative(x,y)}
...
lsimmodellist<-sapply(lsimmodellist,function(x,dummy_data,dummy_phyltree){
## phylogenies are all of height 1
	...
	x
},dummy_data=dummy_data,dummy_phyltree=dummy_phyltree,simplify=FALSE)
## =================================================================================================================================
