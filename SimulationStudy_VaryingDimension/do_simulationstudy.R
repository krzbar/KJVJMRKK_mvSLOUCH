...
sapply(lmodelcomparison_setups,function(lsetup,vsetupstorun,runnum,main_directory,b_should_random_seed_be_saved,b_use_random_seed_from_manuscript,cl){
if (vsetupstorun[[lsetup$id]]){
	...
	sapply(lsetup$simulsetup$N,function(N,lsetup,runnum,main_directory,setuprun_directory,b_should_random_seed_be_saved,b_use_random_seed_from_manuscript,cl){
	#sapply(lsetup$simulsetup$N,function(N,lsetup,runnum,main_directory,setuprun_directory,b_should_random_seed_be_saved,b_use_random_seed_from_manuscript){
	#parSapply(cl,lsetup$simulsetup$N,function(N,lsetup,runnum,main_directory,setuprun_directory,b_should_random_seed_be_saved,b_use_random_seed_from_manuscript){
	    #source("tosource_for_mvSLOUCH.R")
	    ape_phyltree<-NA
	    lres<-NA
		...
		lres<-parSapply(cl,v_iter,function(i,N,lsetup,setup_comments_file,setup_error_comments_file,setuprun_directory,setuprun_directory_tmp,b_use_random_seed_from_manuscript,b_should_random_seed_be_saved){
        	    source("tosource_for_mvSLOUCH.R")
			...
			res_i<-NA
				    ...
				    bm_indep=mvSLOUCH::simulBMProcPhylTree(ape_phyltree,X0=lsetup$simulsetup$modelparams$vX0,Sigma=lsetup$simulsetup$modelparams$Sxx,M.error=Merror),
			    ...
			    s_time<-proc.time()
	    		    ...
	    		    mvsl_estim_res<-mvSLOUCH::estimate.evolutionary.model(ape_phyltree, mData, regimes = NULL, 
            		    ...
            		    model_setups<-lsetup$recoverunder$model_setups
            		    if (is.element("non_mvSLOUCH_model_setups",names(lsetup$recoverunder))&&(!is.null(lsetup$recoverunder$non_mvSLOUCH_model_setups))&&(length(lsetup$recoverunder$non_mvSLOUCH_model_setups)>0)){
            			## additional models that are not present in mvSLOUCH
            			non_mvsl_estim_res<-list(testedModels=sapply(lsetup$recoverunder$non_mvSLOUCH_model_setups,function(non_mvSLOUCH_setup,mData,ape_phyltree){
            			    switch(lsetup$simulsetup$evolmodel,
					bm_indep=BrownianMotionModel_independent_estim_res(ape_phyltree,mData,M.error=Merror),
					NA				
    				    )
            			},mData=mData,ape_phyltree=ape_phyltree,simplify=FALSE),model.setups=lsetup$recoverunder$non_mvSLOUCH_model_setups,repeats=lsetup$recoverunder$repeats,BestModel=NA)
            			## for now we do not consider BestModel, it will be found in the joining procedure            			                			
            			mvsl_estim_res<-join_estimation_results(list(mvsl_estim_res,non_mvsl_estim_res))
            			model_setups<-c(lsetup$recoverunder$model_setups,lsetup$recoverunder$non_mvSLOUCH_model_setups)
            		    }
			    runtime<-proc.time()-s_time
			    lextracted_estim_summary<-extract_estimation_summary(lsetup$simulsetup,model_setups,mvsl_estim_res)
	    		    true_param_summary<-switch(lsetup$simulsetup$evolmodel,
		    		    ...
		    		    bm_indep=mvSLOUCH::SummarizeBM(phyltree=ape_phyltree, mData=mData, modelParams=lextracted_estim_summary$TruePoint_model, t = c(1),M.error=Merror),
	    ...
	    NA
#	},lsetup=lsetup,runnum=runnum,main_directory=main_directory,setuprun_directory=setuprun_directory,b_should_random_seed_be_saved=b_should_random_seed_be_saved,b_use_random_seed_from_manuscript=b_use_random_seed_from_manuscript,simplify=FALSE)
	},lsetup=lsetup,runnum=runnum,main_directory=main_directory,setuprun_directory=setuprun_directory,b_should_random_seed_be_saved=b_should_random_seed_be_saved,b_use_random_seed_from_manuscript=b_use_random_seed_from_manuscript,cl=cl,simplify=FALSE)
    }
    NA   
...
},vsetupstorun=vsetupstorun,runnum=runnum,main_directory=main_directory,b_should_random_seed_be_saved=b_should_random_seed_be_saved,b_use_random_seed_from_manuscript=b_use_random_seed_from_manuscript,cl=cl,simplify=FALSE)
