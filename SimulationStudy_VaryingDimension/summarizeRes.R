...
## function returns a matrix that counts how many times the row model is preferred over the column model under AICc
## on the diagonal it would be a comparison of same models, hence not relevant and 0 is kept
...
f_checksamemodel<-function(model1,model2){
	...
	if (b_isSame){
		...
		if (is.null(model1[[i1]]) && (!is.null(model1[[i1]]))){b_isSame<-FALSE}
			...
			if ((is.null(model2[[i2]]))||(!identical(model1[[i1]],model2[[i2]]))){b_isSame<-FALSE}
		    }
		}
	    }else{b_isSame<-FALSE}
	}
