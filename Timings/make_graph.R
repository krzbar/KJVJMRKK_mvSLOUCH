...
vN<-c(5,10,25,50,100,200,400) ## number of tips for plotting 
vcolours<-c("gray","red","blue","green","orange","brown") ## these are for BM, OUOU, OUBM with different mvSLOUCH versions
...
plot(NA,xlim=c(0,max(vN)+1),ylim=c(0,(max(l_timings[[which(names(l_timings)==paste0("n_",max(vN)))]]$mTimings[c(3,5),],na.rm=TRUE))),main="",xlab="n",ylab="time[s]")
...
par(fig=c(0.3,0.9, 0.45,1), new=TRUE, las=1, ps=9,bty="n")
plot(NA,xlim=c(0,max(vN)+1),ylim=c(0,max(apply(l_timings[[which(names(l_timings)==paste0("n_",max(vN)))]]$mTimings[c(1,2,4,6),,drop=FALSE],1,median,na.rm=TRUE),na.rm=TRUE)+50),main="",xlab="n",ylab="time[s]")
